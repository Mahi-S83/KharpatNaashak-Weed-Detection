...
# ============================================
# KHARPATNAASHAK - COMPLETE WEED DETECTION SYSTEM (ALL FEATURES FIXED)
# Features: Hindi toggle | Video | Heatmaps | PDF Reports | Cost Savings
# ============================================
...
print("="*60)
print("🌾 KHARPATNAASHAK - COMPLETE WEED DETECTION SYSTEM")
print("="*60)
...
print("\n📦 Installing packages...")
!pip install ultralytics gradio fpdf opencv-python matplotlib pillow reportlab -q

# STEP 3: Copy dataset
print("\n📁 Copying dataset...")
import os
if not os.path.exists('/content/mh_weed_yolo'):
    !cp -r /content/drive/MyDrive/mh_weed_yolo /content/ 2>/dev/null
    if not os.path.exists('/content/mh_weed_yolo'):
        !cp -r /content/drive/MyDrive/mh_weed_project/mh_weed_yolo /content/ 2>/dev/null
    print("   ✅ Dataset copied")
else:
    print("   ✅ Dataset already present")

# STEP 4: Use the NEW 100-epoch model
model_path = "/content/drive/MyDrive/mh_weed_project/yolov11_mh_weed16_100epochs/weights/best.pt"
if not os.path.exists(model_path):
    model_path = "/content/drive/MyDrive/mh_weed_project/yolov11_mh_weed16_final/weights/best.pt"
print(f"\n🤖 Model: {model_path}")
...
def process_image(image, confidence=0.3, show_heatmap=False, field_area=1, lang='en'):
    ...
    # Build summary with cost savings
...
def process_video(video_file, confidence=0.3, lang='en'):
...
# Store last results for PDF generation
last_results = {"detections": [], "total": 0, "savings": (0,0,0,0)}

def process_multiple_with_details(files, conf, heatmap, field_area=1):
    """Process multiple images and return gallery, summary, heatmaps, and data for PDF"""
    gallery_images = []
    heatmap_images = []
    all_detections = {}
    total_weeds_all = 0
    ...
    for f in files:
        ...
        if det_count > 0:
            all_weed_lists.append(detections)

    # Create summary text with cost savings
...
with gr.Blocks(title="KharpatNaashak - Weed Detection", theme=gr.themes.Soft()) as demo:
    ...
    gr.HTML("""<div style="text-align: center; padding: 1rem; background: linear-gradient(135deg, #2E7D32, #1B5E20); border-radius: 10px;">
        ...
        <p style="color: #FFD700; font-size: 0.9rem;">Model: 100 Epochs | mAP@0.5: 57.1% | Precision: 64.4% | Recall: 53.1%</p>
    </div>""")
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("📸 Single Image"):
            with gr.Row():
                with gr.Column():
                    ...
                    conf_slider = gr.Slider(0.1, 0.9, 0.3, step=0.05, label="Confidence Threshold")
                    with gr.Row():
        ...
        with gr.TabItem("🎬 Video"):
            with gr.Row():
                with gr.Column():
                    ...
                    video_conf = gr.Slider(0.1, 0.9, 0.3, step=0.05, label="Confidence Threshold")
        ...
        # Tab 3: Multiple Images with Heatmaps, Summary, and PDF
        with gr.TabItem("📚 Multiple Images"):
            with gr.Row():
                with gr.Column():
                    files_in = gr.File(file_count="multiple", label="Upload Multiple Images")
                    multi_conf = gr.Slider(0.1, 0.9, 0.3, step=0.05, label="Confidence Threshold")
        ...
        with gr.TabItem("ℹ️ About"):
            ...
            **Training:** 100 epochs
            **Performance:** mAP@0.5: 57.1% | Precision: 64.4% | Recall: 53.1%
    ...
    detect_btn.click(
        detect_wrapper,
        [img_in, conf_slider, heatmap_chk, field_area_single, lang_selector],
        [img_out, heatmap_out, summary_out, detections_out, stats_out]
    )
    ...
    process_vid_btn.click(
        video_wrapper,
        [video_in, video_conf, lang_selector],
        [video_out, video_summary]
    )
    ...
    process_all_btn.click(
        process_and_display,
        [files_in, multi_conf, multi_heatmap, multi_area],
        [gallery_out, multi_summary]
    )
...
print("\n" + "="*60)
print("✅ KHARPATNAASHAK IS RUNNING with ALL FEATURES FIXED!")
print("📱 Features: Hindi/English | Video Upload | Heatmaps | PDF Reports | Cost Savings")
print("📊 Multiple Images: Heatmaps + Summary + Cost Savings + PDF Report")
print("="*60)
